...
function run() {
  ...
  return PBStore.open()
    ...
    .then(function () { return gate('FA24_NO_S11_WORK', function () {
      // S11-A-R2/S11-B: FA24 originally asserted "no S11-named file exists" — valid only at the
      // S10-FINAL QA moment, before S11-A/S11-B were GPT-authorized and implemented. It is now a
      // strict allowlist instead: the specifically authorized S11-A + S11-B footprint passes, but
      // any other S11-named file (S11-C/D/E/F/FINAL, or an unexpected extra S11-A/S11-B file)
      // still fails — the governance purpose ("no unauthorized next-stage work") is preserved,
      // not removed.
      ...
      [].concat(jsFiles, testFiles, docFiles).forEach(function (f) {
        if (/s11/i.test(f)) {
          assert.ok(authorizedS11Files.indexOf(f) !== -1, 'only the GPT-authorized S11-A/S11-B footprint may exist, unauthorized S11-named file: ' + f);
        }
      });
      authorizedS11Files.forEach(function (f) {
        var found = jsFiles.indexOf(f) !== -1 || testFiles.indexOf(f) !== -1 || docFiles.indexOf(f) !== -1;
        assert.ok(found, 'authorized S11-A/S11-B file is present: ' + f);
      });
      var storageSrc = stripComments(readSrc('js/storage.js'));
      assert.ok(!/DB_VERSION\s*=\s*6/.test(storageSrc), 'no DB_VERSION 6');
    }); })
