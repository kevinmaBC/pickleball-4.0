 ...
 *   - no new IndexedDB store, DB_VERSION unchanged (still 3);
...
function run() {
  return PBStore.open().then(function () {
    ...
    assert.strictEqual(PBStore.DB_VERSION, 3, 'DB_VERSION must stay 3 — S9-A adds no new store');
...
function createFakeStoreCheck() {
  ...
  return new Promise(function (resolve, reject) {
    ...
    assert.strictEqual(stores.length, expected.length, 'S9-A must not create any new object store (expected exactly the 12 existing stores, found: ' + stores.join(', ') + ')');
